 ...
 */
exports.batchRiver = (input, maxSize, timeout = null, handler) => new River((resolve, reject, write, free) => {
  const ms = ~~timeout;
  let batch = {};
  let size = 0;
  let timer;
  ...
  let fResolve;
  let fWrite;
  const feedback = new River((resolve, _, write) => { fResolve = resolve; fWrite = write; });
