...
const { option, omit } = require('./utils');
...
const paginated = (method, lastPage, nextPage, itemList) => function paginatedMethod(params) {
  return new River((resolve, reject, write, free) => {
    const itemsOnly = !!option(params, 'itemsOnly');
    const originalLimit = option(params, 'Limit');
    let total = 0;
    let op = method;
    free(() => { op = undefined; });
    params = omit(params, 'itemsOnly');
      ...
      .then(() => op && op.call(this.db, params)
        .then((result) => {
          ...
          if (originalLimit) {
            ...
            if (total === originalLimit) return;
            params.Limit = originalLimit - total;
          }
