 ...
 *    from dynamo.
 */
function iterateDynamo(op, params) {
  return new River((resolve, reject, write) => {
    params = Object.assign({}, params);

    function performOperation(ExclusiveStartKey) {
      params.ExclusiveStartKey = ExclusiveStartKey;
      return op(params).then((data) => {
        write(data);
        if (data.LastEvaluatedKey) return performOperation(data.LastEvaluatedKey);
        return undefined;
      });
    }

    performOperation().then(resolve, reject);
  });
}
...
function riverDynamo(op, params) {
  return iterateDynamo(op, params)
